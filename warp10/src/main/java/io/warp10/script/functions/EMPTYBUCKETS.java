...
/**
 * Apply emptyBuckets on GTS instances
 */
public class EMPTYBUCKETS extends NamedWarpScriptFunction implements WarpScriptStackFunction {

  public EMPTYBUCKETS(String name) {
    super(name);
  }
  ...
  public Object apply(WarpScriptStack stack) throws WarpScriptException {
    ...
    if (top instanceof List) {
      ...
      for (GeoTimeSerie gts: series) {
        result.add(GTSHelper.emptyBuckets(gts));
      }
      stack.push(result);
    } else if (top instanceof GeoTimeSerie) {
      stack.push(GTSHelper.emptyBuckets((GeoTimeSerie) top));
    } else {
      throw new WarpScriptException(getName() + " expects a LIST or a Geo Time Series as input.");
    }

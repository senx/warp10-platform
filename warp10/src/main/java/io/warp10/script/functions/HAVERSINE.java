...
public class HAVERSINE extends NamedWarpScriptFunction implements WarpScriptStackFunction {
  ...
  public Object apply(WarpScriptStack stack) throws WarpScriptException {
    ...
    double sinphi2 = Math.sin((rlon2 - rlon1)/2.0D);
    sinphi2 *= sinphi2;

    double sinlambda2 = Math.sin((rlat2 - rlat1)/2.0D);
    sinlambda2 *= sinlambda2;

    double d = 2.0D * EARTH_RADIUS * Math.asin(Math.sqrt(sinphi2 + Math.cos(rlon1) * Math.cos(rlon2) * sinlambda2));

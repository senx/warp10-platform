...
public class REDUCE extends NamedWarpScriptFunction implements WarpScriptStackFunction {
  ...
  public Object apply(WarpScriptStack stack) throws WarpScriptException {
    ...
    if (null != params.get(labelsIndex) && !(params.get(labelsIndex) instanceof Collection<?>)) {
      throw new WarpScriptException(getName() + " expects a list of label names or null as the parameter number" + labelsIndex + 1 + ".");
    } else {
      if (null != params.get(labelsIndex)) {
        for (Object o: ((Collection<?>) params.get(labelsIndex))) {
          if (!(o instanceof String)) {
            throw new WarpScriptException(getName() + " expects a list of label names as the parameter number" + labelsIndex + 1 + ".");
          }
        }        
      }
    }
    ...
    if (!(params.get(reducerIndex) instanceof WarpScriptReducerFunction)) {
      throw new WarpScriptException(getName() + " expects a function as the parameter number" + reducerIndex + 1 + ".");
    }

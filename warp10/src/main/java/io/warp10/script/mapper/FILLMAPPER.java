 ...
 * A gts that has been bucketized by the function BUCKETIZE have ticks at regular timestamps (bucketends) separated by the same time span (bucketspan).
 * Some of those ticks are not associated with a value. They are gap, aka missing values, in the bucketized gts.

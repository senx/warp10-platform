...
public class StandaloneParallelStoreClientWrapper implements StoreClient {
  ...
  @Override
  public GTSDecoderIterator fetch(ReadToken token, List<Metadata> metadatas, long now, long timespan, boolean fromArchive, boolean writeTimestamp, final int preBoundary, final int postBoundary) throws IOException {
    if (ParallelGTSDecoderIteratorWrapper.useParallelScanners()) {
      return new ParallelGTSDecoderIteratorWrapper(parent, token, now, timespan, metadatas, preBoundary, postBoundary);
    } else {
      return parent.fetch(token, metadatas, now, timespan, fromArchive, writeTimestamp, preBoundary, postBoundary);
    }
  }

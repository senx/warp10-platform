...
class TestWriteToken extends TokenTestCase {
  ...
  //
  public void testGenerateAndDecodeStandard() {
  ...
  public void testGenerateAndDecodeWithLabels() {
    // generate a valid token with
    // owner = producer
    String owner = UUID.randomUUID().toString()
    String producer = UUID.randomUUID().toString()
  ...
  //
  public void testTokenAESCorrupted() {
    // generate a token
    String uuid = UUID.randomUUID().toString()
  ...
  public void testTokenB64Corrupted() {
    // generate a token
    String uuid = UUID.randomUUID().toString()
    String writeToken = tokenEncoder.deliverWriteToken("app", uuid, 32478, getKeyStore())
